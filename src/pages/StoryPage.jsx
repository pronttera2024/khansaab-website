...
function MobileTimelineItem({ e, i, total }) {
  ...
  return (
    <li ref={ref} style={{
      position: 'relative', paddingBottom: 44,
      opacity: visible ? 1 : 0,
      ...
      <p className="mono" style={{ color: 'var(--gold-light)', opacity: 0.75, fontSize: 10, letterSpacing: '0.22em', marginBottom: 6 }}>
        CHAPTER {String(i + 1).padStart(2, '0')}
      </p>
      <p className="display-italic" style={{ fontSize: 48, color: 'var(--gold)', lineHeight: 0.9, marginBottom: 14, letterSpacing: '-0.02em' }}>{e.year}</p>
      <div style={{ aspectRatio: '16/9', marginBottom: 16, position: 'relative', overflow: 'hidden' }}>
        <Img variant="dark" label={e.img} style={{ height: '100%' }}/>
        <div style={{
          position: 'absolute', bottom: 8, right: 8,
          padding: '4px 8px', background: 'var(--gold)', color: 'var(--ink)',
          fontFamily: 'var(--f-display)', fontStyle: 'italic', fontSize: 13,
        }}>
          {String(i + 1).padStart(2, '0')} / {String(total).padStart(2, '0')}
        </div>
      </div>
      <h3 className="display" style={{ fontSize: 26, marginBottom: 8, fontWeight: 400, lineHeight: 1.1 }}>{e.title}</h3>
...
function MobileTimeline() {
  return (
    <section style={{ background: 'var(--ink)', color: 'var(--ivory)', padding: '72px 0 96px', position: 'relative', overflow: 'hidden' }}>
      <div className="geo-overlay" style={{ opacity: 0.05 }}/>
      <div className="container" style={{ position: 'relative' }}>
        <header style={{ textAlign: 'center', marginBottom: 48 }}>
          <p className="eyebrow" style={{ color: 'var(--gold)', marginBottom: 12 }}>FIFTY-FOUR YEARS</p>
          <h2 className="display" style={{ fontSize: 'clamp(34px, 9vw, 56px)', lineHeight: 1.05, fontWeight: 400 }}>
        ...
        <ol style={{ listStyle: 'none', position: 'relative', paddingLeft: 28 }}>
          <span aria-hidden style={{
            ...
            width: 1, background: 'linear-gradient(180deg, rgba(201,169,97,0.5), rgba(201,169,97,0.15) 90%, transparent)',
          }}/>
          {TIMELINE_EVENTS.map((e, i) => (
            <MobileTimelineItem key={i} e={e} i={i} total={TIMELINE_EVENTS.length}/>
          ))}
        </ol>
      </div>
    </section>
...
function TeamCard({ member }) {
  ...
  return (
    <article onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} style={{ cursor: 'pointer', position: 'relative' }}>
      <div style={{ position: 'relative', aspectRatio: '3/4', marginBottom: isPhone ? 12 : 20, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(180deg, transparent 35%, rgba(10,9,8,${open ? 0.92 : 0.55}) 100%)`, transition: 'background 0.5s' }}/>
        <div style={{ position: 'absolute', bottom: isPhone ? 14 : 24, left: isPhone ? 14 : 24, right: isPhone ? 14 : 24, color: 'var(--ivory)' }}>
          <p className="arabic" style={{ fontSize: isPhone ? 16 : 22, color: 'var(--gold)', marginBottom: 4 }}>{member.arabic}</p>
          <h3 className="display" style={{ fontSize: isPhone ? 18 : 26, marginBottom: 4, lineHeight: 1.1 }}>{member.name}</h3>
          <p className="mono" style={{ opacity: 0.7, fontSize: isPhone ? 9 : 11, letterSpacing: '0.18em' }}>{member.role}</p>
          {!isPhone && (
        ...
        <div style={{
          position: 'absolute', top: isPhone ? 10 : 16, right: isPhone ? 10 : 16,
          padding: isPhone ? '4px 7px' : '6px 10px',
          background: 'rgba(245,239,227,0.92)', color: 'var(--ink)',
          fontSize: isPhone ? 9 : 10, fontFamily: 'var(--f-mono)', letterSpacing: '0.15em',
        }}>
          SINCE {member.since}
        </div>
      </div>
      {isPhone && (
        <p className="display-italic" style={{ fontSize: 13.5, lineHeight: 1.5, opacity: 0.78, color: 'var(--emerald)' }}>"{member.quote}"</p>
      )}
    </article>
  )
}
...
function Team() {
  const { isPhone } = useViewport()
  return (
    <section style={{ padding: isPhone ? '80px 0 72px' : '160px 0', background: 'var(--paper)' }}>
      <div className="container">
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: isPhone ? 40 : 80, flexWrap: 'wrap', gap: isPhone ? 14 : 24 }}>
          <div>
            <p className="eyebrow" style={{ color: 'var(--emerald)', marginBottom: 14 }}>FORTY HANDS</p>
            <h2 className="display" style={{ fontSize: 'clamp(40px, 9vw, 104px)', lineHeight: 1.02, fontWeight: 400 }}>
              The <span className="display-italic" style={{ color: 'var(--emerald)' }}>house.</span>
            </h2>
          </div>
          <p style={{ maxWidth: 380, opacity: 0.65, fontSize: isPhone ? 14 : 16, lineHeight: 1.6 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isPhone ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: isPhone ? 14 : 24 }}>
          {TEAM.map((m, i) => <TeamCard key={i} member={m}/>)}
        </div>
      </div>
      <div style={{ marginTop: isPhone ? 60 : 120, height: isPhone ? '46vh' : '70vh', position: 'relative' }}>
        <Img label="THE FULL ATELIER · 40 ARTISANS · GROUP PORTRAIT · DUBAI 2026" style={{ height: '100%' }}/>
        <div style={{ position: 'absolute', bottom: isPhone ? 18 : 36, left: 0, right: 0, textAlign: 'center', color: 'var(--ivory)', padding: '0 16px' }}>
          <p className="mono" style={{ opacity: 0.85, fontSize: isPhone ? 10 : 11, letterSpacing: '0.22em' }}>THE FULL ATELIER · DUBAI · MARCH 2026</p>
        </div>
      </div>
    </section>
  )
    ...
    <main style={{ background: 'var(--ivory)' }}>
      ...
      <section style={{
        ...
        <div className="container" style={{ textAlign: 'center' }}>
          <p className="eyebrow" style={{ color: 'var(--emerald)', marginBottom: isPhone ? 14 : 20 }}>EST. MMXIV · DUBAI</p>
          <p className="arabic" style={{ fontSize: isPhone ? 26 : 36, color: 'var(--emerald)', marginBottom: 14, opacity: 0.8 }}>تراثنا قصتنا</p>
          <h1 className="display" style={{
            fontSize: 'clamp(54px, 14vw, 220px)',
            lineHeight: 0.92, fontWeight: 400, letterSpacing: '-0.02em',
          ...
          <p style={{
            ...
            fontSize: isPhone ? 15 : 18, lineHeight: 1.7, opacity: 0.7,
          ...
          {isPhone && (
            <p className="mono" style={{ opacity: 0.5, fontSize: 10, letterSpacing: '0.3em', marginTop: 28 }}>↓  SCROLL TO BEGIN</p>
          )}
        </div>
        {!isPhone && (
          <>
      ...
      <section style={{ position: 'relative', height: isPhone ? '70vh' : '80vh' }}>
        ...
        <div style={{
          ...
          <div className="mono" style={{ opacity: 0.75, fontSize: isPhone ? 10 : 11, letterSpacing: '0.22em', color: 'var(--gold-light)' }}>
          ...
          <div style={{
            ...
            fontSize: isPhone ? 18 : 16, lineHeight: 1.45,
            opacity: 0.95,
          }}>
      ...
      <section style={{ padding: isPhone ? '80px 0' : '160px 0', background: 'var(--paper)' }}>
        <div className="container" style={{ maxWidth: 980 }}>
          <div style={{ textAlign: 'center', marginBottom: isPhone ? 44 : 80 }}>
            <p className="eyebrow" style={{ color: 'var(--emerald)', marginBottom: 14 }}>A LETTER FROM THE ATELIER</p>
            <h2 className="display" style={{ fontSize: 'clamp(30px, 8vw, 80px)', lineHeight: 1.05, fontWeight: 400 }}>
              <RevealText text="From a single sewing room to forty pairs of hands." />
            </h2>
          </div>
          <div data-letter style={{
            ...
            <p style={{ marginBottom: 22 }}>
              <span className="display" style={{
                fontSize: isPhone ? 64 : 80, float: 'left',
                lineHeight: 0.85, marginRight: 12, marginTop: 4,
                color: 'var(--emerald)', fontWeight: 400,
              }}>I</span>
            ...
            <p style={{ marginBottom: 0 }}>
              We do not run advertisements. We do not chase seasons. We make heritage garments for men who know the difference.
            </p>
          </div>
          <div style={{
            ...
            paddingTop: 24, borderTop: '1px solid rgba(10,9,8,0.12)',
          }}>
            <div style={{
            ...
            <div>
              ...
              <p className="mono" style={{ opacity: 0.55, fontSize: 10, letterSpacing: '0.2em' }}>MASTER TAILOR · THIRD GENERATION</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{
        ...
        <div style={{ position: 'relative', height: isPhone ? '60vh' : '100%' }}>
          <Img label="HAND CUTTING · CHALK MARKS · POPLIN" style={{ height: '100%' }}/>
          {isPhone && <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, transparent 50%, rgba(245,239,227,0.85) 100%)' }}/>}
          <div style={{ position: 'absolute', bottom: isPhone ? 20 : 32, left: isPhone ? 20 : 32, color: 'var(--ink)', maxWidth: 280 }}>
            <p className="mono" style={{ opacity: 0.7, marginBottom: 6, fontSize: isPhone ? 10 : 11, letterSpacing: '0.22em' }}>CHAPTER I</p>
            <p className="display" style={{ fontSize: isPhone ? 32 : 28, lineHeight: 1.05 }}>The Cloth.</p>
          </div>
        </div>
        <div style={{ position: 'relative', height: isPhone ? '60vh' : '100%' }}>
          <Img variant="dark" label="GOLD THREAD EMBROIDERY · COLLAR DETAIL · MACRO" style={{ height: '100%' }}/>
          {isPhone && <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, transparent 50%, rgba(10,9,8,0.85) 100%)' }}/>}
          <div style={{ position: 'absolute', bottom: isPhone ? 20 : 32, left: isPhone ? 20 : 32, color: 'var(--ivory)', maxWidth: 280 }}>
            <p className="mono" style={{ opacity: 0.85, marginBottom: 6, color: 'var(--gold)', fontSize: isPhone ? 10 : 11, letterSpacing: '0.22em' }}>CHAPTER II</p>
            <p className="display" style={{ fontSize: isPhone ? 32 : 28, lineHeight: 1.05 }}>The Thread.</p>
          </div>
        </div>
      </section>
